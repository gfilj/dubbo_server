...
apps.controller("nodesDetail",function($scope,$httpWrapper,$routeParams,$queryFilter,$breadcrumb,$menu,$dialog){
    $menu.switchMenu("admin/apps");
    $scope.details=[];
    $scope.isEmpty=false;
    $scope.application=$routeParams.application;
    ...
    $scope.batchDeleteDynamicConfig=function(nodeAddress){
    	var ipAndPort = nodeAddress.split(':');
    	if(ipAndPort.length>0){
    		$dialog.confirm({
                ...
                callback:function(){
                    $httpWrapper.post({
                        url:"override/batch/deletedynamicconfig.htm" ,
                        data:"ip="+ipAndPort[0]+"&application="+$scope.application,
                        config:{ headers: { 'Content-Type': 'application/x-www-form-urlencoded'}},
                        success:function(data){
                            if(data.result==ajaxResultStatu.SUCCESS){
                                $dialog.info({
                                    content:"成功删除指定节点上的所有动态配置！",
                                    size:"small",
                                    callback:function(){
                                    	$httpWrapper.post({
                                            url: "app/"+$scope.application+"/nodes.htm",
                                            success:function(data){
                                                $scope.details=data;
                                                if(!data||data.length<=0){
                                                    $scope.isEmpty=true;

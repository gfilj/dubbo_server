...
public class OverrideServiceImpl extends AbstractService implements OverrideService {
	...
	public List<Long> listByIP(String ip, String application) {
		List<Long> ids = new ArrayList<Long>();
		if (ip == null || "".equals(ip) || application == null) {
			return ids;
		}
		ConcurrentMap<String, Map<Long, URL>> serviceUrls = getServiceByCategory(Constants.CONFIGURATORS_CATEGORY);
//		System.out.println("serviceUrls.size()  is" + serviceUrls.size());
		...
		for (Map<Long, URL> urlMap : urlMaps) {
			for (Map.Entry<Long, URL> urlEntry : urlMap.entrySet()) {
				...
				/*System.out.println("configApplication is " + configApplication);
				System.out.println("url.getHost() is " + url.getHost());*/
				if (ip.equals(url.getHost())
						&& (Constants.ANY_VALUE.equals(configApplication) || application.equals(configApplication))) {
					ids.add(urlEntry.getKey());
				}
			}
		}
		return ids;
	...
	public List<com.dubboclub.dk.admin.model.Override> listByProvider(Provider provider) {
		...
		for (Map<Long, URL> urlMap : urlMaps) {
			for (Map.Entry<Long, URL> urlEntry : urlMap.entrySet()) {
				URL url = urlEntry.getValue();
				if (Constants.ANYHOST_VALUE.equals(url.getHost()) || providerUrl.getHost().equals(url.getHost())) {
					String configApplication = url.getParameter(Constants.APPLICATION_KEY, url.getUsername());
					...
					if (configApplication == null || Constants.ANY_VALUE.equals(configApplication)
						...
						if ((url.getPort() == 0 || URL.valueOf(provider.getUrl()).getPort() == url.getPort())) {
							if (url.getPath().equals(Tool.getInterface(provider.getServiceKey()))
									&& StringUtils.isEquals(url.getParameter(Constants.GROUP_KEY),
											providerUrl.getParameter(Constants.GROUP_KEY))
									&& StringUtils.isEquals(url.getParameter(Constants.VERSION_KEY),
	...
	public List<Override> listByServiceKey(String serviceKey) {
		...
		for (Map<Long, URL> urlMap : urlMaps) {
			for (Map.Entry<Long, URL> urlEntry : urlMap.entrySet()) {
				URL url = urlEntry.getValue();
				if (url.getPath().equals(interfaceName)
						&& StringUtils.isEquals(group, url.getParameter(Constants.GROUP_KEY))
						&& StringUtils.isEquals(version, url.getParameter(Constants.VERSION_KEY))) {
					overrides.add(SyncUtils.url2Override(new Pair<Long, URL>(urlEntry.getKey(), url)));
				}
			}
		}
	...
	public Provider configProvider(Provider provider) {
		...
		for (Override override : overrides) {
			...
			if (overrideUrl.getParameters().containsKey(Constants.DISABLED_KEY)) {
				url = url.addParameter(Constants.ENABLED_KEY,
						!Boolean.parseBoolean(overrideUrl.getParameter(Constants.DISABLED_KEY)));
			}
		}
